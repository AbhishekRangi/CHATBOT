...
{
  const acctnum = agent.parameters.acctnum;
  const client = new MongoClient(url);
  await client.connect();
  const snap = await client.db("BOT").collection("User_details").findOne({acctnum: acctnum});
...
intentMap.set("drop_down - custom", report_issue);
intentMap.set("drop_down", custom_payload);
